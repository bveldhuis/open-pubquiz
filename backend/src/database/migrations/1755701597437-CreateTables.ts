...
export class CreateTables1755701597437 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // Create teams table
        await queryRunner.query(`CREATE TABLE \`teams\` (\`id\` varchar(36) NOT NULL, \`quiz_session_id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`total_points\` int NOT NULL DEFAULT '0', \`joined_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`last_activity\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_teams_quiz_session_id\` (\`quiz_session_id\`), INDEX \`IDX_teams_total_points\` (\`total_points\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

        // Create questions table
        await queryRunner.query(`CREATE TABLE \`questions\` (\`id\` varchar(36) NOT NULL, \`quiz_session_id\` varchar(36) NOT NULL, \`round_number\` int NOT NULL, \`question_number\` int NOT NULL, \`type\` enum ('multiple_choice', 'open_text', 'sequence', 'true_false', 'numerical', 'image', 'audio', 'video') NOT NULL, \`question_text\` text NOT NULL, \`fun_fact\` text NULL, \`time_limit\` int NULL, \`points\` int NOT NULL DEFAULT '1', \`options\` json NULL, \`correct_answer\` text NULL, \`sequence_items\` json NULL, \`media_url\` varchar(500) NULL, \`numerical_answer\` decimal(15,4) NULL, \`numerical_tolerance\` decimal(15,4) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_questions_quiz_session_id\` (\`quiz_session_id\`), INDEX \`IDX_questions_round_question\` (\`round_number\`, \`question_number\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

        // Create answers table
        await queryRunner.query(`CREATE TABLE \`answers\` (\`id\` varchar(36) NOT NULL, \`question_id\` varchar(36) NOT NULL, \`team_id\` varchar(36) NOT NULL, \`answer_text\` text NOT NULL, \`is_correct\` tinyint NULL, \`points_awarded\` int NOT NULL DEFAULT '0', \`submitted_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`IDX_answers_question_id\` (\`question_id\`), INDEX \`IDX_answers_team_id\` (\`team_id\`), UNIQUE INDEX \`IDX_answers_team_question\` (\`team_id\`, \`question_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`session_events\` (\`id\` varchar(36) NOT NULL, \`quiz_session_id\` varchar(36) NOT NULL, \`event_type\` enum ('session_created', 'question_started', 'question_ended', 'round_started', 'round_ended', 'session_ended', 'start_question', 'end_question', 'show_leaderboard', 'show_review', 'next_round') NOT NULL, \`event_data\` json NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), INDEX \`IDX_session_events_quiz_session_id\` (\`quiz_session_id\`), INDEX \`IDX_session_events_event_type\` (\`event_type\`), INDEX \`IDX_session_events_created_at\` (\`created_at\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

        // Create themes table
        await queryRunner.query(`CREATE TABLE \`themes\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`description\` text NULL, \`is_active\` tinyint NOT NULL DEFAULT '1', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_themes_name\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

        // Create question_sets table
        await queryRunner.query(`CREATE TABLE \`question_sets\` (\`id\` varchar(36) NOT NULL, \`theme_id\` varchar(36) NOT NULL, \`type\` enum ('multiple_choice', 'open_text', 'sequence', 'true_false', 'numerical', 'image', 'audio', 'video') NOT NULL, \`question_text\` text NOT NULL, \`fun_fact\` text NULL, \`time_limit\` int NULL, \`points\` int NOT NULL DEFAULT '1', \`options\` json NULL, \`correct_answer\` text NULL, \`sequence_items\` json NULL, \`media_url\` varchar(500) NULL, \`numerical_answer\` decimal(15,4) NULL, \`numerical_tolerance\` decimal(15,4) NULL, \`difficulty\` enum ('easy', 'medium', 'hard') NOT NULL DEFAULT 'medium', \`is_active\` tinyint NOT NULL DEFAULT '1', \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_question_sets_theme_type\` (\`theme_id\`, \`type\`), INDEX \`IDX_question_sets_theme_active\` (\`theme_id\`, \`is_active\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

        // Create session_configurations table
        await queryRunner.query(`CREATE TABLE \`session_configurations\` (\`id\` varchar(36) NOT NULL, \`quiz_session_id\` varchar(36) NOT NULL, \`total_rounds\` int NOT NULL DEFAULT '1', \`round_configurations\` json NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), INDEX \`IDX_session_configurations_quiz_session_id\` (\`quiz_session_id\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);

        // Create api_keys table
        await queryRunner.query(`CREATE TABLE \`api_keys\` (\`id\` varchar(36) NOT NULL, \`key_name\` varchar(255) NOT NULL, \`api_key\` varchar(255) NOT NULL, \`permissions\` json NOT NULL, \`is_active\` tinyint NOT NULL DEFAULT '1', \`last_used\` datetime(6) NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), UNIQUE INDEX \`IDX_api_keys_api_key\` (\`api_key\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
